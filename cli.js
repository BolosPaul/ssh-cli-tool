...
function connectToRemote(ip, user, name) {
  // Attempt to use tmux for SSH

  // Rename current tmux session and run SSH
  const ssh = spawn('tmux', ['rename-session', `ssh-${name}`], {
    stdio: 'inherit',
  });

  ssh.on('close', (code) => {
    if (code === 0) {
      const sshConnect = spawn('ssh', [`${user}@${ip}`], { stdio: 'inherit' });

      sshConnect.on('close', (sshCode) => {
        console.log(chalk.yellow(`SSH process exited with code ${sshCode}`));
        clearScreen();
        mainMenu();
      });
    } else {
      console.error(chalk.red(`Failed to rename tmux session. Code: ${code}`));
      console.log(chalk.yellow('Falling back to direct SSH connection...'));
      const fallbackSSH = spawn('ssh', [`${user}@${ip}`], { stdio: 'inherit' });

      fallbackSSH.on('close', (fallbackCode) => {
        console.log(chalk.yellow(`SSH process exited with code ${fallbackCode}`));
        clearScreen();
        mainMenu();
      });
    }
